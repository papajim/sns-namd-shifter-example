...
cat <<EOF > namd2_ID0000003-cont.sh
#!/bin/bash
echo -e "\n#################[Container] Now in pegasus lite container script #################"  1>&2
set -e
# setting environment variables for job
EOF
container_env /scratch >> namd2_ID0000003-cont.sh
cat <<EOF2 >> namd2_ID0000003-cont.sh
...
[
 ...
 { "type": "transfer",
   ...
   "id": 4,
   "src_urls": [
     { "site_label": "cori", "url": "gsiftp://corigrid.nersc.gov/global/cscratch1/sd/papajim/georgepap/pegasus/namd_wf/run0001/00/00/equilibrate_250.conf", "checkpoint": "false" }
   ],
   "dest_urls": [
...
par_all27_prot_lipid.inp:crd.md18_vmd_autopsf.pdb:init.xsc:equilibrate_250.conf:Q42.psf
eof
...
pegasus-kickstart  -n namd2 -N ID0000003 -R cori  -s equilibrate_250.restart.xsc=equilibrate_250.restart.xsc -s equilibrate_250.restart.coor=equilibrate_250.restart.coor -s equilibrate_250.restart.vel=equilibrate_250.restart.vel -L namd_wf -T 2019-01-31T16:21:05-08:00 /opt/namd2 +p 32 equilibrate_250.conf
set -e
...
[
 { "type": "transfer",
   ...
   "dest_urls": [
     { "site_label": "cori", "url": "gsiftp://corigrid.nersc.gov/global/cscratch1/sd/papajim/georgepap/pegasus/namd_wf/run0001/00/00/equilibrate_250.restart.xsc" }
   ] }
 ,
 { "type": "transfer",
   "linkage": "output",
   ...
   "dest_urls": [
     { "site_label": "cori", "url": "gsiftp://corigrid.nersc.gov/global/cscratch1/sd/papajim/georgepap/pegasus/namd_wf/run0001/00/00/equilibrate_250.restart.coor" }
   ] }
 ,
 { "type": "transfer",
   "linkage": "output",
   ...
   "dest_urls": [
     { "site_label": "cori", "url": "gsiftp://corigrid.nersc.gov/global/cscratch1/sd/papajim/georgepap/pegasus/namd_wf/run0001/00/00/equilibrate_250.restart.vel" }
   ] }
]
eof
...
chmod +x namd2_ID0000003-cont.sh
...
shifter --image papajim/namd_image:latest --volume /Volumes/Work/lfs1:/shared-data/:ro;$PWD:/scratch --workdir=/scratch papajim/namd_image:latest ./namd2_ID0000003-cont.sh 
job_ec=$(($job_ec + $?))
